...
async function process(event, context, callback) {
  ...
  const result = [].concat(...(await Promise.all(event.records.map(mapRecord))))

  log.info({ count: result.length }, 'Processing completed')
  ...
  async function mapRecord(record) {
    let payload
    try {
      payload = await gunzip(new Buffer(record.data, 'base64'))
    } catch (err) {
      log.error({ err }, 'Failed to decompress record')
      failure++
      return {
    ...
    } else if (parsed.messageType === 'DATA_MESSAGE') {
      const { logEvents } = parsed
      return {
        recordId: record.recordId,
        result: 'Ok',
        data: Buffer.from(
          logEvents.map(({ message }) => message + '\n').join('')
        ).toString('base64')
      }
    } else {
      return {
...
// Lambda Blueprint from AWS follows
/*
"""
